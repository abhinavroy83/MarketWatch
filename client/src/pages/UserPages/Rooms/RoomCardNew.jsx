...
function RoomcardNew({ isRoomOnlyPage, ...item }) {
  ...
  const makewishlist = async (_id) => {
    ...
    try {
      ...
      // console.log(dat);
      const res = await axios.post(`http://api.verydesi.com/api/addtowish`, dat, {
        headers: {
          jwttoken: `${token}`,
  ...
  const unwish = async (_id) => {
    try {
      const res = await axios.delete(
        `http://api.verydesi.com/api/deletelist/${_id}`
      );
      if (res) {
    ...
    const fetchwishstatus = async () => {
      try {
        const res = await axios.get(
          `http://api.verydesi.com/api/getlistbyroom/${item._id}`
        );
  ...
  return (
    <Link
      to={`/rooms/${item._id}`}
      key={item._id}
      className={`relative max-w-4xl rounded-xl hover:shadow-xl inline-block w-[300px] group`}
    >
      <img
        src={item.PrdImage}
        alt=""
      ...
      >
        {/* <p>{item.postedon}</p> */}
        <h1 className="inline-flex items-center text-[23px] font-['udemy-regular'] mt-2 whitespace-normal text-[#3a3247]">
          {item.Adname && truncateCharacters(item.Adname, 32)}
        </h1>
        {/* <p className="text-[18px] text-gray-500 font-roboto">{item.area}</p> */}
        <p className="flex gap-1 text-[18px] text-gray-600 mt-1 font-['udemy-regular'] items-center">
        ...
        <img
          className="flex absolute bottom-3 right-2"
          height={40}
          width={45}
          src={
            item.gender === "female"
